      ...
      \begin{minipage}{1\linewidth}
        ...
        \FOR{$i=1$ to $|V(G)|$}
          ...
          \IF{$\{L^{(i)}_v : v \in V(G)\} \ne \{L^{(i)}_v : v \in V(H)\}$}
            \RETURN \FALSE
          \ENDIF
          \IF{$\{L^{(i-1)}_v : v \in V(G)\} = \{L^{(i)}_v : v \in V(G)\}$}
            \STATE \textbf{break}
          \ENDIF
        \ENDFOR
        \RETURN \TRUE
...
\begin{frame}
  ...
  \begin{block}{证明：单射图神经网络的表达能力与 WL-Test 等价}
    ...
    下证：对于任意轮迭代 $k$，存在单射函数 $\varphi^{(k)}$，使得 $h_v^{(k)} = \varphi^{(k)}(L_v^{(k)})$。
...
\begin{frame}
  ...
  \frametitle{图神经网络与 WL-Test 的比较}
  \begin{block}{证明：单射图神经网络的表达能力与 WL-Test 等价}
    $k \ge 1$ 时，假设第 $k - 1$ 轮迭代时存在单射函数 $\varphi^{(k-1)}$，则
    \begin{align*}
      h_v^k &= \phi(h_v^{(k-1)}, f(\{h_u^{(k-1)} : u \in \mathcal{N}(v)\})) \\
            ...
            &= \psi \circ g^{-1}(L_v^{(k)})
    \end{align*}
    其中，$\psi = \phi \circ \varphi^{(k-1)}$ 为单射函数。取 $\varphi^{(k)} = \psi \circ g^{-1}$ 即证。
...
\begin{frame}
  ...
  $k > 1$ 时，将 $f^{(k)} \circ \varphi^{(k-1)}$ 看成一个整体，使用多层感知机拟合任意单射函数，得到图同构神经网络的更新公式：
  $$
